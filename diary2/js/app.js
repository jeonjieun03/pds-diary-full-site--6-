...
import { signUp, signIn, signOut, onAuthChange, deleteMyData } from "./auth.js";
...
async function showAppShell(session) {
  ...
  try {
    ...
    banner.hidden = false;
    banner.textContent = "데이터를 불러오지 못했습니다. sql/schema-t07.sql을 실행했는지 확인해주세요.";
  }
}
...
guardSubmit($("#login-form"), async (e) => {
  ...
  try {
    ...
    // 아이디가 없을 때나 비밀번호만 틀렸을 때나 Supabase는 항상 같은 문구
    // ("Invalid login credentials")를 돌려준다 — 계정 존재 여부가 새지 않도록.
...
guardSubmit($("#signup-form"), async (e) => {
  ...
  const f = e.target;
  try {
    await signUp(f.email.value.trim(), f.password.value);
    flashMessageOnAuth("가입되었습니다. 이제 로그인해 주세요.");
    f.reset();
    $("#signup-form").hidden = true;
    $("#login-form").hidden = false;
    $("#show-signup-btn").hidden = false;
  } catch (err) {
    showAuthError(err.message === "User already registered" ? "이미 가입된 이메일입니다." : "가입에 실패했습니다: " + err.message);
  }
});

function flashMessageOnAuth(msg) {
  showAuthError(msg);
  $("#auth-error").style.background = "#eef7ea";
  $("#auth-error").style.borderColor = "var(--moss)";
  $("#auth-error").style.color = "#2f4a27";
}

$("#show-signup-btn").addEventListener("click", () => {
  clearAuthError();
  $("#login-form").hidden = true;
  $("#show-signup-btn").hidden = true;
...
$("#delete-account-btn").addEventListener("click", async () => {
  if (!confirm("정말로 내 자료를 전부 지울까요? 되돌릴 수 없습니다.")) return;
  await deleteMyData();
  await signOut();
});
...
// ---------------- 시작 ----------------
// onAuthChange는 구독 시점에 현재 세션 상태(INITIAL_SESSION)로 한 번 즉시 불려서,
// 로그인 여부에 따라 로그인 화면 / 앱 화면 중 하나로 시작하게 해 준다.
resetPlanForm();
resetTodoForm();
